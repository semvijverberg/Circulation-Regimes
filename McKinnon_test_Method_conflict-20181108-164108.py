...
input.values = np.reshape(smooth_values, (input.time.size, input[0,:,0].size, input[0,0,:].size) )
input.coords['mask'] = (('latitude','longitude'), landseamask)
input.name = 'sst'
threshold = 0.5 * np.std(input, axis = 0)
#threshold = np.ones((input[0].shape)) * np.std(input).values
rep_perc = np.tile(threshold, (input.time.size,1,1))
binary = np.squeeze(input.where(np.squeeze(input.values) > rep_perc))
binary.values = np.nan_to_num(binary)
binary.values[binary.values > 0 ] = 1

#for i in np.linspace(0,binary.time.size-1, 9): xarray_mask_plot(binary[int(i)])
for i in np.linspace(0,input.time.size-1, 9): xarray_plot(input[int(i)])
...
ex['clusmethod'] = methods[1] ; ex['linkage'] = linkage[1] ; region='U.S.'
n_clusters = 4
