...
hotdaythreshold = mcKts.mean(dim='time') + mcKts.std().values
hotts = mcKts.where( mcKts.values > hotdaythreshold.values) 
...
#varfull = varfullspatial.sel(matchdaysmcK=datesmcK)
...
for i in range(3):
    func_mcK.xarray_plot(varhotdays.isel(time=i)) 
