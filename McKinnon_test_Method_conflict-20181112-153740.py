...
datesvar = pd.to_datetime(varfullgl.time.values)
...
def expand_times_for_lags(datesmcK, ex):
    expanded_time = []
    for yr in set(datesmcK.year):
        one_yr = datesmcK.where(datesmcK.year == yr).dropna(how='any')
        start_mcK = one_yr[0]
        #start day shifted half a time step
        half_step = ex['tfreq']/2.
        start_mcK = start_mcK - np.timedelta64(int(half_step), 'D')
        end_mcK   = one_yr[-1] + np.timedelta64(int(half_step), 'D')
        steps = len(one_yr)
        shift_start = start_mcK - steps * np.timedelta64(ex['tfreq'], 'D')
        adj_year = pd.DatetimeIndex(start=shift_start, end=end_mcK, 
                                    freq=(datesmcK[1] - datesmcK[0])).values
...
# Converting Mckinnon timestemp to match xarray timestemp
matchdaysmcK = func_mcK.to_datesmcK(expanded_time, expanded_time[0].hour, varfullgl.time[0].dt.hour)
# region mckinnon - full time series
varfselreg = varfullgl.sel(time=matchdaysmcK)
