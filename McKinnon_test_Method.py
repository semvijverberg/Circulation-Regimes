...
#%%
import clustering_temporal as clus

landseamask = (varhotdays[0].values != 0.0)
input = varhotdays.copy()
input.coords['mask'] = (('latitude','longitude'), landseamask)
input.name = 'sst'
data = input
binary = np.zeros(input.shape)
threshold = np.std(input, axis = 1)
# =============================================================================
...
n_clusters = 4
output = clus.clustering_spatial(input, ex, n_clusters, region)
