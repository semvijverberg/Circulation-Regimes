...
var_clus = functions_pp.Var_import_RV_netcdf(ex)
var_clus = functions_pp.Var_import_RV_netcdf(ex)
ex[ex['vars'][0][0]] = var_clus
functions_pp.perform_post_processing(ex)


#%%
var_clus = ex[ex['var_clusnc_name'][0]]
RV_period = []
for mon in ex['RV_months']:
    # append the indices of each year corresponding to your RV period
    RV_period.insert(-1, np.where(var_clus.dates.month == mon)[0] )
RV_period = [x for sublist in RV_period for x in sublist]
RV_period.sort()
ex['RV_period'] = RV_period
var_clus.datesRV = var_clus.dates[RV_period]

# import array
marray, temperature = functions.import_array(var_clus, path='pp')
...
McKts = functions.Mckin_timeseries(tmaxRVperiod, var_clus)
...
#%%
#ex['RVnc_name'] = [ex['vars'][0][0], '{}_1979-2017_1_12_daily_2.5deg.nc'.format(ex['vars'][0][0])]
ex['RVname'] = ex['vars'][0][1]
file_name = '{}_1979-2017_2jan_31aug_dt-1days_2.5deg.nc'.format(ex['RVname'])
...
output = functions.clustering_temporal(z_data_norm, ex, n_clusters, var_clus, tfreq, region=region)
...
folder = os.path.join(ex['fig_path'],'Clustering_temporal',
                              '{}_{}_{}deg_tfreq{}'.format(RV.name, ex['var_name'],
